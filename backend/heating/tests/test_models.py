...
class ZoneModelTest(TestCase):
    ...
    def test_get_modes_queryset_method(self):
        ...
        self.assertEqual(
            Zone.objects.get_modes(), {'1': 'C', '2': 'E', '3': 'A'})
...
class SlotModelTest(TestCase):
    ...
    def test_string_representation(self):
        ...
        self.assertEqual(
            str(slot),
            "Z{} 04:02:00-15:54:00 [L*M*V*D] Eco".format(slot.zone.num)
        )
...
class DerogationModelTest(TestCase):
    ...
    def test_is_active_model_method(self):
        self.create_entries_for_active_tests()
        self.assertFalse(self.past_derog.is_active())
        self.assertTrue(self.active_derog.is_active())
        self.assertFalse(self.future_derog.is_active())

    def test_outdated_queryset_method(self):
        self.create_entries_for_active_tests()
        queryset = Derogation.objects.outdated()
        self.assertEqual(queryset.count(), 1)
        self.assertEqual(queryset[0], self.past_derog)

    def test_is_outdated_model_method(self):
        self.create_entries_for_active_tests()
        self.assertTrue(self.past_derog.is_outdated())
        self.assertFalse(self.active_derog.is_outdated())
        self.assertFalse(self.future_derog.is_outdated())
